...
struct ClientConnectedMsg
{
    ...
    ClientConnectedMsg(char username[], SocketServerSession* clientSocket) {strcpy(this->username, username); this->clientSocket = clientSocket;}
    ClientConnectedMsg() : username(""), clientSocket(NULL) {}
    char* encode();
    void decode(const char* buffer);
};
...
struct RejectConnectMsg
{
    ...
    RejectConnectMsg(const char* message) {strcpy(this->message, message); messageSize = strlen(message);}
    RejectConnectMsg() : message("") {}
    char* encode();
    void decode(const char* buffer);
};